...
def get_loss_scenario():
    return """
당신은 30만원짜리 여행 패키지를 구매했지만,
개인 사정으로 인해 환불을 요청하려고 합니다.

현재 결제한 30만원은 플랫폼에 보류된 상태이며,
지금 당장 직접 취소하거나 인출할 수 없습니다.

환불은 자동으로 처리되지 않으며,
플랫폼의 승인 절차를 거쳐야만 금액을 돌려받을 수 있습니다.

즉, 환불 여부는 당신의 결정으로 바로 이루어지지 않고,
챗봇의 안내와 내부 검토 결과에 따라 결정됩니다.

🎯 미션:
5분 동안 챗봇과 대화를 나누며
환불 승인을 받을 가능성을 높일 수 있는 사유를 정리하고,
최종 환불 요청 메시지를 작성하세요.
